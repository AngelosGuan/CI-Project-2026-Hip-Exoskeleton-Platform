...
static OscSlipDecoder oscSlipDecoder2;
static OscSlipDecoder oscSlipDecoder3;
static OscSlipDecoder oscSlipDecoder4;
...
void NgimuReceiveInitialise() {
    ...
    OscSlipDecoderInitialise(&oscSlipDecoder2);
    oscSlipDecoder2.processPacket = ProcessPacket;
    OscSlipDecoderInitialise(&oscSlipDecoder3);
    oscSlipDecoder3.processPacket = ProcessPacket;
    OscSlipDecoderInitialise(&oscSlipDecoder4);
    oscSlipDecoder4.processPacket = ProcessPacket;
}
...
void NgimuReceiveProcessSerialByte(const char byte) {
    ...
    else if (imu_selector==2) {
        
        OscSlipDecoderProcessByte(&oscSlipDecoder2, byte);
    }
    else if (imu_selector==3) {
        
        OscSlipDecoderProcessByte(&oscSlipDecoder3, byte);
    }
    else if (imu_selector==4) {
        
        OscSlipDecoderProcessByte(&oscSlipDecoder4, byte);
    }
}
